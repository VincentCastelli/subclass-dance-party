...
describe('fadedDancer', function() {
  ...
  var fadedDancer, clock;
  var timeBetweenSteps = 100;

  beforeEach(function() {
    clock = sinon.useFakeTimers();
    fadedDancer = new makeFadedDancer(10, 20, timeBetweenSteps);
  });
  ...
  it('should have a step function that makes its node blink', function() {
    sinon.spy(fadedDancer.$node, 'toggle');
    fadedDancer.step();
    expect(fadedDancer.$node.toggle.called).to.be.true;
  });
...
describe('shimmyDancer', function() {
  ...
  beforeEach(function() {
    clock = sinon.useFakeTimers();
    shimmyDancer = new makeShimmyDancer(10, 20, timeBetweenSteps);
  });
  ...
  it('should have a step function that makes its node blink', function() {
    sinon.spy(shimmyDancer.$node, 'toggle');
    shimmyDancer.step();
    expect(shimmyDancer.$node.toggle.called).to.be.true;
  });
